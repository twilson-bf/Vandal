...
async function inventoryPage(token){
 ...
 const facetNames={org:'Organization',cve:'CVE',country:'Country',domain:'Domain',coverage:'Coverage',scope:'Scope state',scope_tag:'Scope tag',port:'Port',service:'Service',source:'Source'};
 ...
 const cards=data.items.map(a=>{
  ...
  return `<article class="host-result"><div class="result-header"><input type="checkbox" data-select="${a.id}" data-value="${esc(a.value)}" ${selected.has(a.id)?'checked':''} aria-label="Select ${esc(a.value)}"><h2><a href="${hostLink(a.id)}">${esc(a.value)}</a></h2><span class="coverage ${a.coverage}">${coverageLabel(a.coverage)}</span></div>
   ...
   <div class="result-footer"><a href="${hostLink(a.id)}">Open ${a.kind==='ip'?'host':'hostname'} →</a><span>${a.potential?`${a.potential} potential CVEs · `:''}${a.confirmed?`${a.confirmed} confirmed · `:''}${a.sources.length} sources</span>${a.kind==='ip'?`<button class="quiet" data-quick-scan="${a.id}">Nmap host</button>`:'<button class="quiet" data-host-scan="'+a.id+'" data-host-value="'+esc(a.value)+'">Scan hostname</button>'}</div></article>`;
 }).join('');
...
renderers.host=async function(token){
 ...
 $('#page-actions').innerHTML=a.kind==='ip'?`<button class="primary" data-quick-scan="${a.id}">Nmap host</button>`:`<button class="primary" data-host-scan="${a.id}" data-host-value="${esc(a.value)}">New scan for hostname</button>`;
...
async function openHostPane(id){
 ...
 $('#host-pane-content').innerHTML=`<header class="host-pane-header"><div class="eyebrow">${esc(a.kind)} / ${esc(a.scope)}</div><h2>${esc(a.value)}</h2><p>${coverageLabel(a.coverage)} · Last observation ${esc(stamp(a.last_seen))}</p><div class="form-actions">${a.kind==='ip'?`<button class="primary" data-quick-scan="${a.id}">Nmap host</button>`:`<button class="primary" data-host-scan="${a.id}" data-host-value="${esc(a.value)}">New scan</button>`}<button class="quiet" data-interest-asset="${a.id}">Add review item</button></div></header>
...
renderers.scope=async function(token){
 const [settings,groups]=await Promise.all([api(endpoint('scope/settings')),api(endpoint('scope/groups'))]);
 const groupRows=groups.map(g=>`<article class="scope-group" data-scope-group="${esc(g.tag)}"><header><div><span class="micro">TAGGED GROUP · ${g.rule_count} RULE${g.rule_count===1?'':'S'} · ${g.matched_assets} ASSETS</span><h3>${esc(g.tag)}</h3></div><div class="scope-group-controls"><button type="button" class="${g.action==='include'?'primary':'quiet'}" data-group-action="include">Included</button><button type="button" class="${g.action==='exclude'?'primary':'quiet'}" data-group-action="exclude">Excluded</button><button type="button" class="${g.hidden?'primary':'quiet'}" data-group-hidden="${g.hidden?'false':'true'}">${g.hidden?'Hidden':'Visible'}</button></div></header><details><summary>${g.rule_count} target${g.rule_count===1?'':'s'}</summary>${table(['TARGET',''],g.rules.map(r=>`<tr><td>${esc(r.target)}</td><td><button class="quiet" data-remove-scope="${r.id}">Remove</button></td></tr>`).join(''))}</details></article>`).join('');
 put(`<section class="scope-policy"><div><span class="micro">ENGAGEMENT POLICY</span><h2>${settings.limit_to_included?'Included groups only':'Open inventory'}</h2><p>${settings.limit_to_included?'Only targets matched by included groups can run. Exclusions still win.':'Targets can run without a scope entry. Exclusions still block execution.'}</p></div><label class="scope-switch"><input id="scope-limit" type="checkbox" ${settings.limit_to_included?'checked':''}> Limit scans to included groups</label></section><section class="panel dark-panel"><div class="panel-head"><h3>Create or extend a tagged group</h3></div><form id="bulk-scope-form" class="panel-body"><div class="form-grid"><label>TAG<input name="tag" required maxlength="80" placeholder="EPT inventory"></label><label>STATE<select name="action"><option value="include">Included</option><option value="exclude">Excluded</option></select></label></div><label>TARGETS<textarea name="targets" required placeholder="example.com&#10;192.0.2.0/24&#10;192.0.2.10-192.0.2.40"></textarea></label><label>LOAD TEXT FILE<input type="file" id="scope-list-file" accept=".txt,.csv,text/plain,text/csv"></label><label class="checkbox"><input name="hidden" type="checkbox"> Hide this group from inventory and global selection</label><p class="micro">One entry per line or comma separated. Domains match their subdomains. A tag gives every entry one shared execution and visibility state.</p><button class="primary">Preview group</button><div id="scope-preview"></div></form></section><section class="scope-register"><div class="console-heading"><h2>Tagged groups</h2><span class="micro">${groups.length} GROUPS</span></div>${groupRows||empty('No tagged groups','Inventory is open and unrestricted until you add a group.')}</section>`,token);
 const form=$('#bulk-scope-form');if(!form)return;
 $('#scope-limit').onchange=async e=>{e.target.disabled=true;try{const r=await api(endpoint('scope/settings'),{method:'PATCH',body:{limit_to_included:e.target.checked}});selected.clear();toast(r.cancelled_jobs.length?`${r.cancelled_jobs.length} jobs stopped by scope policy`:'Scope policy updated');await route()}catch(err){e.target.disabled=false;toast(err.message)}};
 document.querySelectorAll('[data-scope-group]').forEach(row=>row.onclick=async e=>{const button=e.target.closest('[data-group-action],[data-group-hidden]');if(!button)return;button.disabled=true;try{const body=button.dataset.groupAction?{action:button.dataset.groupAction}:{hidden:button.dataset.groupHidden==='true'};const r=await api(endpoint('scope/groups/'+encodeURIComponent(row.dataset.scopeGroup)),{method:'PATCH',body});selected.clear();toast(r.cancelled_jobs?.length?`${r.cancelled_jobs.length} jobs stopped by scope policy`:'Group updated');await route()}catch(err){button.disabled=false;toast(err.message)}});
 ...
 form.onsubmit=async e=>{e.preventDefault();try{
  const body=Object.fromEntries(new FormData(form));body.hidden=form.elements.hidden.checked;const preview=await api(endpoint('scope'),{method:'POST',body:{...body,preview:true}});
  $('#scope-preview').innerHTML=`<p>${preview.targets.length} rules · ${preview.matched_assets} matching assets</p><details><summary>Normalized rules</summary><pre>${esc(preview.targets.join('\n'))}</pre></details><button type="button" class="primary" id="apply-scope">Apply to ${esc(body.tag)}</button>`;
