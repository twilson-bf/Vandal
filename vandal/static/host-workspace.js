...
function workspaceTabs(){return `<div class="workspace-signal" aria-hidden="true"><b>SURFACE INDEX</b><span>HOST / DOMAIN / SERVICE / VULNERABILITY</span><i>CHANNEL 01</i></div><div class="workspace-bar"><nav class="workspace-tabs" aria-label="Explore views">${[['explore','Hosts'],['domains','Domains'],['web','Web'],['findings','Vulnerabilities']].map(([v,label])=>`<a href="#${v}" data-workspace-view="${v}">${label}</a>`).join('')}</nav><div class="workspace-preferences" data-workspace-preferences></div></div>`}
...
function hostCard(a){
 ...
 const risk=[a.pwned?`<strong class="risk-pwned">PWNED · ${a.callback_count} beacon${a.callback_count===1?'':'s'}</strong>`:'',a.confirmed?`<strong class="risk-confirmed">${a.confirmed} confirmed</strong>`:'',a.potential?`${a.potential} potential`:'' ].filter(Boolean).join(' · ')||'<span class="muted">No CVE claims</span>';
 ...
 <div class="host-identity result-header"><h2><a href="${hostLink(a.id)}">${esc(a.value)}</a></h2><div>${esc(ip)}${view.addresses.length>1?` <span class="muted">+${view.addresses.length-1}</span>`:''} <span class="address-state">${esc(view.status)}</span></div><small>${esc(a.provider||'Organization unknown')} · ${esc(a.country||'Unknown')}${a.scope_tags?.length?' · '+a.scope_tags.map(esc).join(', '):''}</small></div>
...
function currentFacets(data){
 const labels={domain:'Domain',country:'Country',coverage:'Coverage',scope:'Scope state',scope_tag:'Scope tag',org:'Organization',cve:'CVE',port:'Open port',service:'Service',source:'Source'};
...
renderers.explore=async function(token){
 ...
 put(`<div class="query-aperture"><span class="query-prompt">QUERY://</span><form id="current-host-search" class="toolbar"><input name="q" class="search" aria-label="Search hosts" placeholder="hostname:example.com port:443" value="${esc(q)}"><button class="primary">EXECUTE</button><a class="tag" href="${esc(exportUrl)}">EXPORT CSV</a></form><span class="query-channel">INDEX MEMORY / CURRENT EVIDENCE</span></div>
 <details class="search-help"><summary>Search syntax</summary><p>Search current evidence. Examples: <code>hostname:example.com port:443 has:vuln has:pwned scope_tag:"EPT inventory" coverage:scanned</code>. Previous observations are under each host's History.</p></details>
...
function serviceGroup(s,hostId,open=false){return `<details class="current-service" id="current-service-${s.protocol}-${s.port}" ${open?'open':''}><summary><strong>${s.port}/${esc(s.protocol)}</strong> ${badge(s.state)} <span>${esc(s.service||'Service unknown')} · ${s.variants.length} owner${s.variants.length===1?'':'s'}</span></summary>${s.variants.map(v=>`<section class="service-variant"><h3>${esc(v.owner)} · ${esc(v.service||'Unknown service')}</h3><p>${badge(v.evidence)} · ${esc(stamp(v.observed_at))}</p><p>${esc(v.product)}</p><div class="raw-channel"><span>RAW SERVICE OUTPUT // RECORD ${v.record_id}</span><pre class="host-banner">${esc(v.banner||'No source output')}</pre></div><a class="text-link" href="#" data-record="${v.record_id}">Current evidence #${v.record_id}</a></section>`).join('')}<p class="micro">${s.count} retained observations. See History for superseded evidence.</p></details>`}
...
openHostPane=async function(id){
 ...
 try{
  ...
  const callbackLinks=a.mythic_callbacks.map(c=>`<article class="mythic-callback"><div><strong>#${c.display_id} · ${esc(c.user||'?')}@${esc(c.host||'?')}</strong>${c.active?badge('active'):badge('inactive')}</div><p>${esc(c.payload_type||'Mythic callback')} · ${esc(c.os||'OS unknown')} · ${esc(stamp(c.last_seen||c.first_seen))}</p><p class="micro">MATCH: ${c.match_basis.map(esc).join(' + ')} · ${esc(c.source_name)}</p><a class="primary external-link" href="${esc(c.source_url)}" target="_blank" rel="noopener noreferrer">Open in Mythic ↗</a></article>`).join('');
  $('#host-pane-content').innerHTML=`<div class="host-top-row"><header class="host-pane-header ${a.pwned?'is-pwned':''}"><div class="host-serial" aria-hidden="true">${a.pwned?'PWNED':'H'+String(a.id).padStart(5,'0')}</div><div class="eyebrow">HOST #${a.id} · ${esc(a.scope)}</div><h2>${esc(a.value)}</h2><div class="host-register"><span>${coverageLabel(a.coverage)}</span>${a.pwned?`<span class="pwned-state">PWNED · ${a.callback_count} BEACON${a.callback_count===1?'':'S'}</span>`:''}<span>${a.open_port_count} open services</span><span>${esc(a.address_view.primary_ip||'Unresolved')}</span><span>${esc(a.address_view.status)}</span></div><pre class="identity-aperture"><i>asset.identity</i> = ${esc(a.value)}\n<i>current.addr</i>   = ${esc(a.address_view.primary_ip||'UNRESOLVED')}\n<i>dns.state</i>      = ${esc(a.address_view.status)}\n<i>evidence.open</i>  = ${a.open_port_count}\n<i>mythic.beacons</i> = ${a.callback_count}</pre><div class="form-actions"><button class="quiet" data-interest-asset="${a.id}">Add item</button><button class="quiet" data-reload-host="${a.id}">Refresh</button>${a.kind==='hostname'?`<button class="quiet" data-sync-dns="${esc(a.value)}">Sync DNS</button>`:`<button class="quiet" data-reverse-dns="${esc(a.value)}">Validate hostname</button>`}</div></header><aside class="host-confirmed ${confirmed.length||manual.length?'has-confirmed':''}"><h3>Confirmed vulnerabilities <span>${confirmed.length+manual.length}</span></h3>${confirmedSummary||'<p class="muted">None confirmed.</p>'}</aside></div>
  <div class="host-pane-grid"><div class="host-facts"><section class="panel"><div class="panel-head"><h3>Current identity</h3></div><dl class="pane-facts"><dt>Primary address</dt><dd>${esc(a.address_view.primary_ip||'Unresolved')}</dd><dt>Addresses</dt><dd>${a.address_view.addresses.map(p=>`<a href="${hostLink(p.id)}">${esc(p.value)}</a>`).join('<br>')||'—'}</dd><dt>DNS status</dt><dd>${esc(a.address_view.status)}</dd><dt>Last validated</dt><dd>${esc(stamp(a.address_view.checked_at))}</dd><dt>Scope tags</dt><dd>${a.scope_tags?.map(tag=>`<a href="#explore?${new URLSearchParams({q:'scope_tag:'+JSON.stringify(tag)})}">${esc(tag)}</a>`).join('<br>')||'—'}</dd><dt>Organization</dt><dd>${esc(a.provider||'Unknown')}</dd><dt>Location</dt><dd>${esc(a.country||'Unknown')} / ${esc(a.city||'—')}</dd><dt>ASN</dt><dd>${esc(a.asn||'—')}</dd></dl></section><section class="panel host-vulnerability-register"><div class="panel-head"><h3>Vulnerabilities · ${vulnerabilities.length} unique CVEs</h3></div><div class="panel-body">${vulnerabilities.map(vulnerabilityGroup).join('')||'<p class="muted">No current CVE claims.</p>'}${a.other_findings.length?`<details><summary>Other items (${a.other_findings.length})</summary>${claimLinks(a.other_findings)}</details>`:''}</div></section><details><summary>Notes and tags</summary><form id="asset-form" data-id="${a.id}"><label>Tags<input name="tags" value="${esc(a.tags)}"></label><label>Notes<textarea name="notes">${esc(a.notes)}</textarea></label><button class="primary">Save</button></form></details></div>
  <div class="host-services"><section class="panel"><div class="panel-head"><h3>Current services</h3></div><div class="panel-body">${a.services.map(s=>`<a class="tag" href="#" data-current-jump="${s.protocol}-${s.port}">${s.port}/${esc(s.protocol)} · ${esc(s.state)}</a>`).join(' ')||'No current service evidence'}</div></section>${a.services.map((s,index)=>serviceGroup(s,a.id,index===0)).join('')}${hostHistory(a)}<details><summary>Scan history (${a.jobs.length})</summary>${jobRows(a.jobs)}</details></div></div>`;
  if(a.pwned)$('#host-pane-content .host-facts').insertAdjacentHTML('afterbegin',`<section class="panel pwned-register"><div class="panel-head"><h3>Mythic beacons · ${a.callback_count}</h3></div><div class="panel-body">${callbackLinks}</div></section>`);
